 ...
 * @since 2.0
 */
class Collapse extends \yii\bootstrap\Widget
{
  /**
  ...
  {
    parent::init();
    Html::addCssClass($this->options, ['widget' => 'box-group']);
  }
  ...
  public function renderItems()
  {
    ...
    foreach ($this->items as $key => $item) {
      ...
      $header = $item['label'];
      $tools = '';
      if (array_key_exists('tools', $item)){
        $tools = $item['tools'];
      }

      $options = ArrayHelper::getValue($item, 'options', []);
      Html::addCssClass($options, ['panel' => 'panel box', 'widget' => 'box-primary']);
      $items[] = Html::tag('div', $this->renderItem($header, $item, ++$index,$tools), $options);
    }
   ...
   */
  public function renderItem($header, $item, $index,$tools)
  {
    if (array_key_exists('content', $item)) {
      ...
      $header = Html::tag('h4', $headerToggle, ['class' => 'box-title']).Html::tag('div',$tools,['class'=>'box-tools pull-right']);
      ...
      if (is_string($item['content']) || is_numeric($item['content']) || is_object($item['content'])) {
        ...
        if (isset($item['footer'])) {
          $content .= Html::tag('div', $item['footer'], ['class' => 'box-footer']) . "\n";
        }
      } else {
    ...
    $group[] = Html::tag('div', $header, ['class' => 'box-header with-border']);
